...
const Billing: React.FC = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => { if (event.target?.result) setPreviewPhoto(event.target.result as string); };
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    // Set category explicitly for bill payments
    await payBill(selectedBill.id, amount, paymentMethod, selectedBankId, paymentProof || undefined, undefined, isEditMode, paymentDate, 'PENERIMAAN TAGIHAN');
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[160] p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[90vh]">
             ...
             <form onSubmit={handlePaymentSubmit} className="p-8 space-y-5 pt-4 overflow-y-auto custom-scrollbar">
                ...
                {(paymentMethod === 'TRANSFER' || isEditMode) && (<div className="space-y-2 animate-in fade-in slide-in-from-top-2"><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('lbl_proof')} <span className="text-rose-500">*Wajib</span></label><input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileSelect}/>{previewPhoto ? (<div className="relative rounded-2xl overflow-hidden border border-slate-200 group"><img src={previewPhoto} alt="Preview" className="w-full h-32 object-cover" /><div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" onClick={() => setPreviewPhoto(null)} className="bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors"><Trash2 size={16} /></button></div></div>) : (<div onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-2xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all ${isResident ? 'border-blue-200 bg-blue-50 hover:border-blue-300' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`}><Upload size={24} className={isResident ? "text-blue-400" : "text-slate-400"} /><p className="text-[10px] font-bold text-slate-500 mt-2 uppercase tracking-widest">{t('btn_upload')}</p></div>)}{isResident && !previewPhoto && <p className="text-[9px] text-rose-500 font-bold italic">* Mohon lampirkan foto bukti pembayaran.</p>}</div>)}
