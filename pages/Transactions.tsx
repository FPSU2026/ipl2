...
const Transactions: React.FC = () => {
  ...
  // Filter bank accounts based on transaction type (Income/Expense)
  const availableBankAccounts = useMemo(() => {
      if (formData.type === 'INCOME') return bankAccounts.filter(acc => acc.isActiveForBilling);
      return bankAccounts.filter(acc => acc.isActiveForExpense);
  ...
  // --- LOGIC SALDO AWAL (BREAKDOWN & TOTAL) ---
  const balanceBreakdown = useMemo(() => {
      ...
      // 1. Calculate Cash at Cutoff
      const initialCashBase = jan2026CashTx ? jan2026CashTx.amount : 0;
      let calculatedCash = initialCashBase;
      
      const prevCashTransactions = transactions.filter(t => {
          return t.date >= systemStartDateStr && t.date < cutoffDateStr && t.category !== 'Saldo Awal' && t.paymentMethod === 'CASH';
      });

      prevCashTransactions.forEach(t => {
          const factor = t.type === 'INCOME' ? 1 : -1;
          calculatedCash += (t.amount * factor);
      });

      // 2. Calculate Bank Balances at Cutoff
      const bankDetails: Record<string, number> = {};
      
      bankAccounts.forEach(acc => {
          let balAtCutoff = acc.balance;
          
          const transactionsSinceCutoff = transactions.filter(t => {
              return t.date >= cutoffDateStr && t.paymentMethod === 'TRANSFER' && t.bankAccountId === acc.id;
          });

          transactionsSinceCutoff.forEach(t => {
              const reverseFactor = t.type === 'INCOME' ? -1 : 1;
              balAtCutoff += (t.amount * reverseFactor);
          });
          
          bankDetails[acc.id] = balAtCutoff;
      });
  ...
  const beginningBalance = useMemo(() => {
    return balanceBreakdown.cash + Object.values(balanceBreakdown.bankDetails).reduce((a: number, b: number) => a + b, 0);
  }, [balanceBreakdown]);
  ...
  const handleExportPDF = () => {
    ...
    autoTable(doc, {
        ...
        columnStyles: { 0: { cellWidth: 15, halign: 'center' }, 1: { halign: 'left' }, 2: { cellWidth: 40, halign: 'right' } }
    });
    ...
    const boxWidth = 50;
    const boxHeight = 35;
    const marginX = (pageWidth - (boxWidth * 3)) / 4;
    ...
    doc.text("Admin", x1 + boxWidth/2, yPos + boxHeight - 5, { align: 'center' });
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm self-start lg:self-auto">
                <button onClick={() => setFilterMode('MONTHLY')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${filterMode === 'MONTHLY' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                    <Calendar size={14} /> Bulanan
                </button>
                <button onClick={() => setFilterMode('RANGE')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${filterMode === 'RANGE' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            {filterMode === 'MONTHLY' ? (
                ...
                <div className="bg-white p-2 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 self-start lg:self-auto">
                    <div className="flex items-center gap-2">
                        ...
                        <input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="bg-slate-50 px-3 py-2 rounded-lg text-xs font-bold text-slate-700 outline-none border border-slate-200" />
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end: e.target.value})} className="bg-slate-50 px-3 py-2 rounded-lg text-xs font-bold text-slate-700 outline-none border border-slate-200" />
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center space-x-2 self-start lg:self-auto">
                ...
                <button onClick={handleExportPDF} className="bg-rose-50 text-rose-600 px-4 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 border border-rose-100 hover:bg-rose-100 transition-colors" title="Ekspor PDF Rekap Pengeluaran">
                    ...
                    <td className="px-8 py-5">
                        {t.paymentMethod === 'TRANSFER' ? (
                            <div className="flex flex-col items-start gap-1">
                                <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest">Transfer</span>
                                {bank && (
                            ...
                            <span className="text-slate-600 bg-slate-100 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest">Tunai</span>
                        )}
                    </td>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
              <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in duration-200">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-indigo-50">
                      <div><h3 className="font-black text-indigo-900 text-lg uppercase tracking-tight">Rincian Saldo Awal</h3></div>
                      <button onClick={() => setShowBalanceDetailModal(false)} className="p-2 hover:bg-indigo-100 rounded-full text-indigo-400 transition-colors"><X size={20}/></button>
                  </div>
                  <div className="p-6 space-y-4">
                      {/* Tunai / Cash Section */}
                      <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                          <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Tunai / Cash</span>
                          <span className="text-lg font-black text-slate-800">Rp {balanceBreakdown.cash.toLocaleString('id-ID')}</span>
                      </div>
                      
                      {/* Bank Accounts Section */}
                      <div className="space-y-2 mt-6">
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1 mb-3">REKENING BANK (POSISI SAAT INI)</p>
                          <div className="space-y-2">
                            {bankAccounts.map(acc => (
                                <div key={acc.id} className="flex justify-between items-center p-3 px-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-indigo-200 transition-colors">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-black text-slate-800 uppercase tracking-tight">{acc.bankName} {acc.accountNumber}</span>
                                        <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{acc.accountHolder}</span>
                                    </div>
                                    <span className="font-black text-slate-800 text-sm">Rp {(balanceBreakdown.bankDetails[acc.id] || 0).toLocaleString('id-ID')}</span>
                                </div>
                            ))}
                          </div>
                      </div>
                      ...
                      <div className="pt-6 mt-4 border-t border-slate-100 flex justify-between items-center">
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Total Saldo Awal</span>
                          <span className="text-xl font-black text-indigo-600">Rp {beginningBalance.toLocaleString('id-ID')}</span>
                      </div>
                  </div>
              </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
           <div className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in duration-200">
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-5">
                 ...
                 {formData.paymentMethod === 'TRANSFER' && (<div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Pilih Bank</label><select required value={formData.bankAccountId} onChange={(e) => setFormData({...formData, bankAccountId: e.target.value})} className="w-full p-3 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl font-bold outline-none"><option value="">-- Pilih Rekening --</option>{availableBankAccounts.map(acc => (<option key={acc.id} value={acc.id}>{acc.bankName} - {acc.accountNumber}</option>))}</select></div>)}
                 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-slate-800 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
      ...
      {showInitialBalanceModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
           <div className="bg-white rounded-[2rem] shadow-2xl max-md w-full overflow-hidden animate-in zoom-in duration-200">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-indigo-50">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in duration-200 text-center p-8">
                ...
                <div className="space-y-3">
                    <button onClick={() => importFileRef.current?.click()} disabled={isImporting} className="w-full py-3 bg-slate-800 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-slate-500/20 hover:bg-slate-900 flex items-center justify-center gap-2">
                        {isImporting ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}
                        <span>Pilih File Excel</span>
                    </button>
                    <button onClick={downloadTemplate} className="w-full py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 flex items-center justify-center gap-2">
                        <Download size={14} /> Download Template
                    </button>
                    <button onClick={() => setShowImportModal(false)} className="w-full py-3 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-600">
                        Batal
                    </button>
                </div>
            </div>
